...
🔴 مهم جداً: تاريخ اليوم هو {today_ar} ({today_str}).
...
5. عدم إضافة معلومات غير موجودة في الخبر الأصلي — خصوصاً التواريخ!
...
def _summarize_articles_sync(articles: List[Dict]) -> List[Dict]:
    ...
    now = datetime.now()
    today_str = f"{now.year}-{now.month:02d}-{now.day:02d}"
    system_prompt = f"أنت مساعد عربي متخصص في أخبار الذكاء الاصطناعي. تجيب دائماً بالعربية الفصحى فقط. تاريخ اليوم: {today_str}. ماتخترعش تواريخ مش موجودة في الأخبار الأصلية."

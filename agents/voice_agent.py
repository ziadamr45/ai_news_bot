...
يدعم: Google Speech (أساسي - مجاني وموثوق) → Groq Whisper → OpenRouter Whisper → OpenAI Whisper
...
class VoiceAgent:
    ...
    def _convert_ogg_to_wav(self, audio_bytes: bytes) -> bytes:
        ...
        try:
            ...
            logger.error(f"❌ Audio conversion error (OGG→WAV): {e}")
            raise

    def transcribe(self, audio_bytes: bytes, file_path: str = None, language: str = "ar") -> dict:
        """
        تحويل الصوت إلى نص — 4 طبقات fallback
        1. Google Speech Recognition (مجاني، موثوق، مش محتاج API key)
        2. Groq Whisper (سريع بس بيقع أحياناً)
        ...
        # Layer 1: Google Speech (free, reliable)
    ...
    def _transcribe_google(self, audio_bytes: bytes, file_path: str = None, language: str = "ar") -> dict:
        """تحويل الصوت لنص باستخدام Google Speech Recognition (مجاني، بدون API key)"""
        try:
        ...
        try:
            ...
            if audio_bytes:
                ...
                if not wav_bytes:
                    ...
                    return {"text": "", "success": False, "error": "conversion failed", "provider": "google"}
            ...
            try:
                ...
                try:
                    ...
                    text = loop.run_in_executor(
                        None,
                        lambda: recognizer.recognize_google(audio_data, language=lang_code)
                    )
                    # Since we can't easily await in sync context, use sync call
